...
SECRET = "7d16162998eb7rfafq1498f75290a937"
...
def make_pycas_cookie(val, domain, path, secure, expires=None):
	cookie = "%s=%s;domain=%s;path=%s" % (PYCAS_NAME, val, domain, path)
	if secure:
		cookie += ";secure"
	if expires:
...
def decode_cookie(cookie_val,lifetime=None):
	#  Test for no cookies
	if cookie_val=="" or cookie_val==None:
		return COOKIE_NONE, ""
...
#  Check pycas cookie
def get_cookie_status(cookies):
	#cookies = get_cookies(environ)
	return decode_cookie(cookies.get(PYCAS_NAME))


def get_ticket_status(cas_host,service_url,protocol,opt,ticket):
...
# Lifetime defines the lifetime of the cookie, security level determines https, protocol determines which CAS protocol to use,
# Path dictates where on the service url to go, and opt gives any options.
def login(cas_host, service_url, cookies, ticket, lifetime=None, secure=1, protocol=2, path="/", opt=""):
	...
	cookie_status, id = get_cookie_status(cookies)
	...
	ticket_status, id = get_ticket_status(cas_host,service_url,protocol,opt,ticket)
