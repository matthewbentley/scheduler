...
\begin{itemize}
\item Class HAS MeetingTime (1:N, full participation from MeetingTime to Has)
\subitem This relationship is satisfied by two pieces: first, each MeetingTime has a foreign key for the id of the class to which it is attached. This satisfies the 1:N part of the relationship. Second, the id is listed as NOT NULL, which enforces the full participation.
\item User Enrollment Event (M:N)
\subitem This relationship is satisfied by the inclusion of the Enrollment table, which contains a user id and an event id.
\item Instructor Instructs MeetingTime (1:N, full participation from Instructor to MeetingTime)
\subitem This relationship is satisfied the inclusion of the Instructs table which has a MeetingTime id and an instructor id. We do not enforce the constraints here because we wanted a separate Instructs relation for extensibility and simplicity. 
\item MeetingTime IS-A Event 
\subitem The IS-A relationship here is satisfied by having each MeetingTime contain an event\_id which connects it to a specific instance of an event. It thus becomes an instance of that event.
\item CustomEvent IS-A Event
\subitem The IS-A relationship here is satisfied by having each CustomEvent contain an event\_id which connects it to a specific instance of an event. It thus becomes an instance of that event.
\end{itemize}
...
\begin{enumerate}[i.]
\item RA: $(\Pi_{id}(enrolls/\Pi_{id}(\sigma_{name=prof}Instructors\bowtie Instructs))-\Pi{id}(Enrolls-(Enrolls\int\Pi_{id}(\sigma_{name=prof}Instructors\bowtie Instructs)))$
...
WHERE NOT EXISTS (
    SELECT *
    FROM MeetingTime m, Class c, Instructs i
    WHERE i.instructor_name = "Professor X" AND i.meeting_id = m.id
          AND m.meeting_class_id = c.id
          AND NOT EXISTS (
                SELECT *
                FROM Enrollment e, MeetingTime m2
                WHERE e.student_id = s.id AND e.meeting_id = m2.id
                        AND m2.meeting_course_id = c.id
                )
    )
AND NOT EXISTS (
    SELECT *
    FROM Enrollment e, MeetingTime m, Instructs i
    WHERE e.student_id = s.id AND e.meeting_id = m.id AND i.meeting_id = m.id
            AND i.instructor_name != "Professor X" AND
            NOT EXISTS (
                SELECT *
                FROM Instructs i2, Instructor p2, MeetingTime m2
                WHERE i2.instructor_id = p2.id AND p2.name = "Professor X"
                        AND i2.meeting_id = m2.id 
                        AND m2.meeting_course_id = m.meeting_course_id
            )
)   
\end{verbatim}
\end{enumerate}
\item Query: Find the names of the instructors who teach the most EECS courses.\\
\begin{enumerate}[i.]
\item RA:$\Pi_{name}(Instructors\bowtie (\gamma_{tid, MAX(cidcount)}(\gamma_{tid, COUNT(cid) AS cidcount}(Instructs))$
\item TRC: $\{t^1\vert(\exists i)(Instructors(p) \wedge t[1]=p.name \wedge (\forall p2)(Instructors(p2) \wedge p2.name<>p.name \rightarrow (\{COUNT(t)|(Instructs(t) \wedge t.instructor=p.name \wedge (\exists m)MeetingTime(m) \wedge t.meeting=m.id (\exists c)Class(c) \wedge m.meeting\_class=c.id \wedge c.dept="EECS")\} \ge \{COUNT(i)|(Instructs(i) \wedge i.instructor=p2.name \wedge (\exists m)MeetingTime(m) \wedge i.meeting=m.id (\exists c)Class(c) \wedge m.meeting\_class=c.id \wedge c.dept="EECS")\}  )))\})$
\item SQL:
\begin{verbatim}
SELECT t1.instructor_name
FROM (
        SELECT i.instructor_name, COUNT(i.instructor_name) AS n_classes
        FROM Instructs i, MeetingTime m, Class c
        WHERE i.meeting_id = m.id AND m.meeting_class_id = c.id
                AND c.dept = "EECS"
        GROUP BY i.instrutor_name
     ) t1
WHERE NOT EXISTS (
    SELECT *
    FROM (
    ...
    WHERE t2.n_classes > t1.n_classes
    )
\end{verbatim}
\end{enumerate}
\item Query: Find the names of instructors who teaches any offering taken by srl51 after 3:00pm and don't teach any offering taken by jrk126
\begin{enumerate}[i.]
\item RA:$ \Pi_{tid}(Instructs\bowtie\Pi_{cid}(\sigma_{id=srl51}Students\bowtie(Enrollment\bowtie$\\
$\Pi_{cid}(\sigma_{start\_time>1500}MeetingTime)))- \Pi_{tid}(Instructs\bowtie$\\
$\Pi_{cid}(\sigma_{id=jrk126}Students\bowtie Enrollment)))$
...
WHERE
    EXISTS (
        SELECT *
        FROM Enroll e, MeetingTime m, Instructs i
        WHERE e.student_id = "sr151" AND e.meeting_id = m.id AND i.meeting_id = m.id
                AND TIMESTAMPDIFF(MINUTE, m.start_time, '15:00:00') > 0
                AND i.instructor_id = p.name
            )
    AND
    NOT EXISTS (
        SELECT *
        FROM Enroll e, MeetingTime m, Instructs i
        WHERE e.student_id = "jrk126" AND e.meeting_id = m.id AND i.meeting_id = m.id
                AND i.instructor_id = p.name
        )

\end{verbatim}
\end{enumerate}
\item Query: Find the names of the teachers and ids of the students who are busy at the same time on Friday, time wise.
\begin{enumerate}[i.]
\item RA: $ \Pi_{tname, sid}(Instructors\bowtie (Instructs\bowtie(\sigma_{"F" IN recur\_type}Courses))\bowtie_{cid}(Enrollment\bowtie(\sigma_{"F" IN recur\_type}Courses))$
...
\item SQL:
\begin{verbatim}
SELECT s.case_id and p.name
FROM Student s, Instructor p
WHERE EXISTS (
        SELECT *
        ...
        WHERE EXISTS (
                SELECT *
                FROM Enroll e
                WHERE e.student_id = s.case_id AND e.event_id = e1.id
                )
            AND EXISTS (
                SELECT *
                FROM Instructs i
                WHERE i.instructor_id = p.name AND i.meeting_id = e2.id
            )
            AND e1.recur_type LIKE '%F%' AND e2.recur_type LIKE '%F%' AND
            (TIMESTAMPDIFF(MINUTE, e1.start_time, e2.start_time) >= 0 AND
            TIMESTAMPDIFF(MINUTE, e1.end_time, e2.start_time) <= 0) OR
            (TIMESTAMPDIFF(MINUTE, e2.start_time, e1.start_time) >= 0 AND
            TIMESTAMPDIFF(MINUTE, e2.end_time, e1.start_time) <= 0)
        )
\end{verbatim}
\end{enumerate}
\item Query: Find the names of the classes on Thursday that srl51 can't take because the custom event "band practice" conflicts with the class.
\begin{enumerate}[i.]
\item RA: $\Pi_{classname}(\sigma_{"Th" IN recur\_type}MeetingTime)$\\
$\bowtie_{(MT.start\_time > CE.start\_time AND MT.start\_time < CE.end\_time)OR}$\\
$_{(MT.end\_time < CE.end\_time AND MT.end\_time > CE.start\_time)}$\\
$\Pi_{start\_time, end\_time}(\sigma_{sid=srl51}Students\bowtie (\sigma_{name="Band Practice"}CustomEvent))$
\item TRC: $\{t^1\vert(\exists c)(\exists e)(\exists n)(Class(c) \wedge CustomEvent(e) \wedge Enrollment(n) \wedge t[1]=c.classname \wedge n.event=e.id \wedge n.student="srl51" \wedge e.event\_name="Band Practice" \wedge Contains(e.recur\_type, 'Th') \wedge (\forall m)(MeetingTime(m) \wedge m.meeting\_class=c.pk \rightarrow Contains(m.recur\_type, 'Th') \wedge ((m.start\_date \geq e.start\_date \wedge m.start\_date \leq e.end\_date) \vee (m.end\_date \leq e.end\_date \wedge m.end\_date \geq e.start\_date)) \wedge ((m.start\_time \geq e.start\_time \wedge m.start\_time \leq e.end\_time) \vee (m.end\_time \leq e.end\_time \wedge m.end\_time \geq e.start\_time))\}$
\item SQL:
\begin{verbatim}
SELECT c.classname
FROM Class c, CustomEvent e
WHERE   e.event_name="band practice"
AND     e.recur_type LIKE 'Th'
AND     EXISTS (SELECT *
                FROM MeetingTime m
                WHERE m.meeting_class.id
                AND m.recur_type LIKE 'Th'
                AND ((m.start_time > e.start_time AND m.start_time < e.end_time)
                    OR (m.start_time < e.start_time AND m.end_tine > e.start_time)))
\end{verbatim}
\end{enumerate}
\end{enumerate}
