...
<td>{{event.meetingtime.meeting\_class.dept}} {{event.meetingtime.meeting_class.class_number}}: {{event.meetingtime.meeting_class.classname}}</td>\\

Note, this is not the exact code we have, because behind the scenes the info view actually iterates through the instructs relation so that we can display an instructor for each meeting time. Also, the <td> tag is an HTML tag for whenever you want to have an entry in a table cell.\\
