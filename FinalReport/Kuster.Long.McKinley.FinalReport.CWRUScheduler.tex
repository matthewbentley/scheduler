...
passed string
\begin{enumerate}[i.]
\item RA: $\Pi_{MeetingTime, instructor}(Instructs\bowtie_{meeting=id}\Pi_{id}(MeetingTime\bowtie_{meeting\_class=pk}\Pi_{pk}(\sigma_{input\_number  IN  dept  OR  input\_number  IN  number  OR  input\_name  IN  classname}Class)))$
...
\begin{enumerate}[i.]
\item RA:$\Pi_{MeetingTime, instructor}(\sigma_{instructor=passed\_name}Instructs)$
