...
from course_scheduler.models import Class, MeetingTime, Instructor, Instructs
...
def format_times(s_time, e_time):
    if "PM" in e_time:
        ...
        if int(e_hour) - int(s_hour) > 6:
            s_hour = add_twelve_hours(s_time)
...
for c in classes:
    ...
    try:
        ...
        if len(desc) > 4096:
            desc = desc[:4095]
        desc = unicode(desc)
        print 'adding:  ' + ' '.join([unicode(c_num), unicode(d), unicode(name), unicode(desc)])
    ...
    for meeting in meetings:
        ...
        m = MeetingTime(meeting_class = c_model, meeting_date = s_date, meeting_end_date = e_date, meeting_recur_type = weekdays, meeting_location = room, meeting_time = s_time, meeting_end = e_time)
        m.save()
